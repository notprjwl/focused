...
const WorkoutDetails = () => {
  const { workouts: workout, loading, error } = useFetch("/api/workouts");
    ...
    <body className='p-2'>
      <div className='text-textTint bg-background pt-2 sm:pt-5 mx-auto rounded-3xl border-[1px] border-text'>
        <div className='items-center overflow-x-hidden'>
          ...
          <table className='table-auto mx-auto sm:w-full md:w-full lg:w-[100%] xl:w-[100%]'>
            <thead className='border-b-[1px] border-text'>
              <tr className='justify-start text-left'>
                <th></th>
                <th className='px-2 py-2 sm:px-3 w-[30%] text-lg sm:text-sm md:text-2xl '>Exercise Name</th>
                <th className='px-2 py-2 sm:px-3 w-[10%] text-lg sm:text-sm md:text-2xl'>Weight</th>
                <th className='px-2 py-2 sm:px-3 w-[10%] text-lg sm:text-sm md:text-2xl'>Sets</th>
                <th className='px-2 py-2 sm:px-3 w-[5%] text-lg sm:text-sm md:text-2xl'>Reps</th>
              </tr>
            </thead>
            <tbody className=''>
              {workout &&
                workout.map((workout) => (
                  <tr key={workout._id} className='text-left border-t-[1px] border-text'>
                    <td className='w-[1%] pl-3'>
                      <input key={workout._id} type='checkbox' className='checkbox text-center align-middle' />
                    </td>
